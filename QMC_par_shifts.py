...
'''
This module implements the Experiment class with 8 threads (one per shift). 
This is intended to run with 8 cores and export OMP_NUM_THREADS=1 to make sure
that FeNiCS does not try to grab too many resources.
'''
...
class Experiment:
    ...
    def parallel_shifts(self, ys, n, k):
        """Process each of the shifts and calculate y^\delta"""
        ...
        new_row = [[ys[0], n, k+1] + new_y.flatten().tolist()]
        ...
        for n in ns:
            ...
            for ys in yss:
                ...
                with ProcessPoolExecutor() as executor:
                    futures = [executor.submit(self.parallel_shifts, ys, n, k) for k in range(q)]
